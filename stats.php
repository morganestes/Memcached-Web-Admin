...
switch ($request) {
    ...
    case 'live_stats':
        ...
        foreach ($_ini->cluster( $cluster ) as $name => $server) {
            ...
            $actual[$name] = Library_Command_Factory::instance( 'stats_api' )->stats( $server['hostname'],
                $server['port'] );
        ...
        foreach ($stats as $server => $array) {
            ...
            if (( isset( $stats[$server]['uptime'] ) ) && ( $stats[$server]['uptime'] > 0 )) {
                ...
                $stats[$server]['bytes_percent']    = sprintf( '%.1f',
                    $actual[$server]['bytes'] / $actual[$server]['limit_maxbytes'] * 100 );
    ...
    default :
        ...
        $stats = array();
        foreach ($_ini->cluster( $cluster ) as $name => $server) {
            $stats[$name] = Library_Command_Factory::instance( 'stats_api' )->stats( $server['hostname'],
                $server['port'] );
        }
        ...
        $refresh_rate = max( $_ini->get( 'refresh_rate' ),
            count( $_ini->cluster( $cluster ) ) * 0.25 + ( Library_Data_Error::count() * ( 0.5 + $_ini->get( 'connection_timeout' ) ) ) );
