        ...
        if (isset( $_GET['request_server'] ) && ( $cluster = $_ini->cluster( $_GET['request_server'] ) )) {
            foreach ($cluster as $server) {
                ...
                echo Library_HTML_Components::serverResponse( $server['hostname'], $server['port'],
                    Library_Command_Factory::api( $_GET['request_api'] )->get( $server['hostname'], $server['port'],
                        $_GET['request_key'] ) );
            }
    ...
    case 'set':
        ...
        if (isset( $_GET['request_server'] ) && ( $cluster = $_ini->cluster( $_GET['request_server'] ) )) {
            foreach ($cluster as $server) {
                # Dumping server get command response
                echo Library_HTML_Components::serverResponse( $server['hostname'], $server['port'],
                    Library_Command_Factory::api( $_GET['request_api'] )->set( $server['hostname'], $server['port'],
                        $_GET['request_key'], $_GET['request_data'], $_GET['request_duration'] ) );
            }
    ...
    case 'delete':
        ...
        if (isset( $_GET['request_server'] ) && ( $cluster = $_ini->cluster( $_GET['request_server'] ) )) {
            foreach ($cluster as $server) {
                # Dumping server get command response
                echo Library_HTML_Components::serverResponse( $server['hostname'], $server['port'],
                    Library_Command_Factory::api( $_GET['request_api'] )->delete( $server['hostname'], $server['port'],
                        $_GET['request_key'] ) );
            }
        ...
        elseif (isset( $_GET['request_server'] ) && ( $server = $_ini->server( $_GET['request_server'] ) )) {
            ...
            foreach ($_ini->get( 'servers' ) as $cluster => $servers) {
                ...
                foreach ($servers as $server) {
                    # Dumping server delete command response
                    echo Library_HTML_Components::serverResponse( $server['hostname'], $server['port'],
                        Library_Command_Factory::api( $_GET['request_api'] )->delete( $server['hostname'],
                            $server['port'], $_GET['request_key'] ) );
                }
            }
        }
        break;
    ...
    case 'increment':
        ...
        if (isset( $_GET['request_server'] ) && ( $cluster = $_ini->cluster( $_GET['request_server'] ) )) {
            foreach ($cluster as $server) {
                ...
                echo Library_HTML_Components::serverResponse( $server['hostname'], $server['port'],
                    Library_Command_Factory::api( $_GET['request_api'] )->increment( $server['hostname'],
                        $server['port'], $_GET['request_key'], $_GET['request_value'] ) );
            }
        } # Ask for increment on one server
        elseif (isset( $_GET['request_server'] ) && ( $server = $_ini->server( $_GET['request_server'] ) )) {
            # Dumping server increment command response
            echo Library_HTML_Components::serverResponse( $server['hostname'], $server['port'],
                Library_Command_Factory::api( $_GET['request_api'] )->increment( $server['hostname'], $server['port'],
                    $_GET['request_key'], $_GET['request_value'] ) );
    ...
    case 'decrement':
        ...
        if (isset( $_GET['request_server'] ) && ( $cluster = $_ini->cluster( $_GET['request_server'] ) )) {
            foreach ($cluster as $server) {
                ...
                echo Library_HTML_Components::serverResponse( $server['hostname'], $server['port'],
                    Library_Command_Factory::api( $_GET['request_api'] )->decrement( $server['hostname'],
                        $server['port'], $_GET['request_key'], $_GET['request_value'] ) );
            }
        ...
        elseif (isset( $_GET['request_server'] ) && ( $server = $_ini->server( $_GET['request_server'] ) )) {
            # Dumping server decrement command response
            echo Library_HTML_Components::serverResponse( $server['hostname'], $server['port'],
                Library_Command_Factory::api( $_GET['request_api'] )->decrement( $server['hostname'], $server['port'],
                    $_GET['request_key'], $_GET['request_value'] ) );
    ...
    case 'flush_all':
        ...
        if (isset( $_GET['request_server'] ) && ( $cluster = $_ini->cluster( $_GET['request_server'] ) )) {
            foreach ($cluster as $server) {
                # Dumping server get command response
                echo Library_HTML_Components::serverResponse( $server['hostname'], $server['port'],
                    Library_Command_Factory::api( $_GET['request_api'] )->flush_all( $server['hostname'],
                        $server['port'], $_GET['request_delay'] ) );
            }
        } # Ask for flush_all on one server
        elseif (isset( $_GET['request_server'] ) && ( $server = $_ini->server( $_GET['request_server'] ) )) {
            # Dumping server flush_all command response
            echo Library_HTML_Components::serverResponse( $server['hostname'], $server['port'],
                Library_Command_Factory::api( $_GET['request_api'] )->flush_all( $server['hostname'], $server['port'],
                    $_GET['request_delay'] ) );
    ...
    case 'search':
        ...
        if (isset( $_GET['request_server'] ) && ( $cluster = $_ini->cluster( $_GET['request_server'] ) )) {
            foreach ($cluster as $server) {
                # Dumping server get command response
                echo Library_HTML_Components::serverResponse( $server['hostname'], $server['port'],
                    Library_Command_Factory::api( 'Server' )->search( $server['hostname'], $server['port'],
                        $_GET['request_key'], $_GET['request_level'], $_GET['request_more'] ) );
            }
    ...
    case 'telnet':
        ...
        if (isset( $_GET['request_server'] ) && ( $cluster = $_ini->cluster( $_GET['request_server'] ) )) {
            foreach ($cluster as $server) {
                ...
                echo Library_HTML_Components::serverResponse( $server['hostname'], $server['port'],
                    Library_Command_Factory::api( 'Server' )->telnet( $server['hostname'], $server['port'],
                        $_GET['request_telnet'] ) );
            }
